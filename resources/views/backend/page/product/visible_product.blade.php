...
@section('title')
<title>Visble Product - {{$settings->company_name}}</title>
@endsection
        ...
        <div class="card">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        <?php $products = DB::table('products')->where('visible','!=',1)->get(); ?>
                        @foreach($products as $product)
                        <tr>
                            <td>{{$product->id}}</td>
                            <td>{{$product->product_code}}</td>
                            <td>{{$product->product_name}}</td>
                            <th><i cat_id="{{$product->id}}" field_name="visible" db_name="products" class="all_hide_show fas fa-toggle-off" style="color: #ed2024; font-size:30px; cursor:pointer"></i></th>
